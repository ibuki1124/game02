...
function drawMark(x, y, mark) {
  if (mark == b){
    // 黒石の描画
    ctx.beginPath();
    ctx.arc(x * cell + 25, y * cell + 25, 20, 0, 2 * Math.PI);
    ctx.fillStyle = "black"
    ctx.fill();
  }else if (mark == w){
    // 白石の描画
...
function checkWinner() {
  ...
  if (num_b > num_w){
    // 黒の勝利
    alert(`黒の勝ちです！`);
    win_count.innerHTML += `<p class="mx-auto border-bottom">${win}戦目.黒<span class="text-danger font-weight-bold">${num_b}</span>：白<span class="text-danger font-weight-bold mr-3">${num_w}</span>黒の勝利</p>`
    win++;
    return true;
  }else if (num_w > num_b){
    // 白の勝利
    alert(`白の勝ちです！`);
    win_count.innerHTML += `<p class="mx-auto border-bottom">${win}戦目.黒<span class="text-danger font-weight-bold">${num_b}</span>：白<span class="text-danger font-weight-bold mr-3">${num_w}</span>白の勝利</p>`
    win++;
    return true;
  }else if (num_b == num_w){
    // 引き分け
    alert(`引き分けです！`);
    win_count.innerHTML += `<p class="mx-auto border-bottom">${win}戦目.引き分け</p>`
    win++;
    return true;
  }
}
